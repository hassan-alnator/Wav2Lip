...
parser.add_argument('--mouth_region_size', type=float, default=0.6,
                    help='Size of mouth region to blend (0.4-0.8)')
...
def create_elliptical_mask(h, w, mouth_region_size=0.6):
    ...
    center_x = w // 2
    center_y = int(h * 0.65)  # Position mask in lower face
    
    # Ellipse dimensions based on face size
    axes_x = int(w * mouth_region_size * 0.5)
    axes_y = int(h * mouth_region_size * 0.35)
    ...
    blur_size = args.blur_intensity if args.blur_intensity % 2 == 1 else args.blur_intensity + 1
    mask = cv2.GaussianBlur(mask, (blur_size, blur_size), blur_size//3)
    
    return mask

def multiscale_blend(pred, original, mask):
...
def edge_aware_blend(pred, original, mask):
    """Apply edge-preserving filter only at blend boundaries"""
    # Initial blend
    mask3 = mask[..., None]
    blended = mask3 * pred + (1 - mask3) * original
    
    # Find edge region (where blending happens)
    edge_mask = (mask > 0.05) & (mask < 0.95)
    
    if edge_mask.any():
        # Apply bilateral filter only to edge region
        filtered = cv2.bilateralFilter(blended.astype(np.uint8), 
                                       d=9, sigmaColor=75, sigmaSpace=75)
        
        # Mix filtered and original based on edge mask
        edge_mask3 = edge_mask[..., None]
        blended = edge_mask3 * filtered + (1 - edge_mask3) * blended
    
    return blended.astype(np.uint8)

def guided_filter_blend(pred, original, mask, r=20, eps=0.01):
    """Fast guided filter for edge-preserving blending"""
    # Initial blend
    mask3 = mask[..., None]
    p = mask3 * pred + (1 - mask3) * original
    
    # Use original as guide
    I = original.astype(np.float32) / 255.0
...
def face_detect(images):
	...
	for i in range(len(results)):
		results[i] = [int(r) for r in results[i]]
...
def datagen(frames, mels):
	...
	for i, m in enumerate(mels):
		...
		img_batch.append(cv2.resize(face, (args.img_size, args.img_size)))
		mel_batch.append(m)
...
def main():
	...
	for i, (img_batch, mel_batch, frames, coords) in enumerate(tqdm(gen, 
		...
		for p, f, c in zip(pred, frames, coords):
			...
			elif args.blend_method == 'ellipse':
				# Fast elliptical mask blending
				h, w = p.shape[:2]
