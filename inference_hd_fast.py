...
def enhance_mouth_region(frame, coords, pred_patch, enhancement_level=1.2):
    """
    FAST HD enhancement - just the essentials that make a difference
    """
    y1, y2, x1, x2 = coords
    target_h, target_w = y2 - y1, x2 - x1
    
    # Just two key steps for REAL improvement without slowdown:
    
    # 1. Better interpolation - CUBIC is visibly better than LINEAR
    upscaled = cv2.resize(pred_patch, (target_w, target_h), interpolation=cv2.INTER_CUBIC)
    
    # 2. Simple but effective sharpening
    if enhancement_level > 0:
        # Unsharp mask - the most important enhancement
        blurred = cv2.GaussianBlur(upscaled, (0, 0), 2.0)
        sharpened = cv2.addWeighted(upscaled, 1.0 + enhancement_level * 1.5, blurred, -enhancement_level * 1.5, 0)
